"""
Skapar Strategiportfoljen_Beskrivning_v315.docx
Fristående dokument — ej kopierat från tidigare version.
Texttitelssida (ingen bild). Utförlig beskrivning av v3.15.
"""
...
dst = fr"{DIR}\Strategiportfoljen_Beskrivning_v315.docx"
...
def fotnot():
    ...
    r = p.add_run("Strategiportföljen v3.15  ·  Byggt för Martin  ·  Strategi från januari 2026")
...
ingress("v3.15: Ny Översikt-flik — cockpit med signaler, nyckeltal och portfölj-accordion")
...
ingress("v3.15: Fem flikar, ett syfte — disciplinerad portföljförvaltning med cockpit-startsida")

punkt(
    "Cockpit med global period-väljare (1D–Allt), 4 nyckeltal (portföljvärde, periodsavkastning, "
    "nettoinsatt, likviditet), portfölj-accordion med inline MA200-redigering, signal-pills per "
    "innehav och kategori-donut med viktbalans-signal i legenden.",
    "▸  Översikt"
)
punkt(
...
punkt(
    "Import, kassa, avstämning, transaktioner, beslutslogg, inställningar och Excel-backup "
    "samlade i Mer-fliken.",
    "▸  Mer"
)
punkt(
...
brödtext(
    "Gå till Mer → Importera. Dra eller välj CSV-filerna. "
    "Importera i ordning: transaktioner → positioner → inköpskurser. "
    "Appen läser dem och bygger upp innehav, transaktioner och kassainformation."
)

rubrik2("Steg 4: Sätt MA200-värden")
brödtext(
    "På Översikt-fliken — klicka på MA200-värdet direkt i portfölj-accordionen. "
    "Ange värdet i lokal valuta och tryck Enter. "
    "Hitta MA200 via din mäklare eller finance.yahoo.com."
)

rubrik2("Steg 5: Konfigurera konton (Inställningar)")
brödtext(
    "Gå till Mer → Inställningar → Kontokonfiguration och verifiera att dina Avanza-konton "
...
brödtext(
    "Gå till Mer → Avstämning efter import. Summorna per konto ska stämma med "
...
brödtext(
    "Gå till Mer → Importera → Backup & Återställning → Ladda ner Excel-backup. "
    "Spara filen i iCloud eller OneDrive. Det är din enda kopia av all data."
)


# ══════════════════════════════════════════════
#  7. NYHETER I VERSION 3.15
# ══════════════════════════════════════════════
sidbyte()
rubrik1("7. Nyheter i version 3.15")
ingress("Cockpit-startsida — allt du behöver se direkt när du öppnar appen")

rubrik2("Ny Översikt-flik")
brödtext(
    "Startsidan är ombyggd till ett cockpit. Navigeringen har fem tydliga flikar: "
    "Översikt, Portfölj, Signaler, Kategorier och Mer. "
    "Översikt är den nya startsidan — allt viktigt samlat på ett ställe."
)

rubrik2("Global period-väljare")
brödtext(
    "En period-väljare högst upp på Översikt (1D / 7D / 30D / 90D / 6M / 1Å / i År / Allt) "
    "styr hela appen. Alla nyckeltal, avkastningsberäkningar och diagram uppdateras "
    "när du väljer en ny period — ett val, ett resultat."
)

rubrik2("Cockpit: 4 nyckeltal")
brödtext(
    "Fyra nyckeltalskort längst upp på Översikt visar direkt:"
)
punkt("Totalt portföljvärde — summan av alla innehav och likvida medel")
punkt("Periodens avkastning — kr och % för vald period")
punkt("Nettoinsatt kapital — vad du faktiskt satt in (exkl. interna kontoöverföringar)")
punkt("Tillgänglig likviditet — kassa tillgänglig för köp per konto")

rubrik2("Portfölj-accordion med inline MA200-redigering")
brödtext(
    "Alla innehav listas per kategori i ett fällbart accordion under nyckeltalskortet. "
    "Varje kategori kan fällas ihop eller ut. "
    "MA200-värdet kan redigeras direkt i listan — klicka på siffran, skriv nytt värde, tryck Enter. "
    "Ingen separat redigeringsdialog behövs."
)

rubrik2("Signal-pills per innehav")
brödtext(
    "Varje rad i accordionen visar en färgad signal-pill: "
    "grön (kurs tydligt över MA200), gul (nära MA200 eller dag 1 under), röd (tvådagarsregel uppfylld). "
    "Håll musen över pillen för en tooltip med exakt status och avstånd."
)

rubrik2("Kategori-donut med viktbalans-signal")
brödtext(
    "Donut-diagrammet på Översikt visar portföljens fördelning per kategori. "
    "Legenden visar för varje kategori: signalcirkel (aggregerad signalstatus), "
    "kategorinamn, pil och procentandel. "
    "Om en kategori avviker från sitt målintervall visas en viktbalans-signal direkt i legenden."
)

rubrik2("Fix: interna Avanza-kontoöverföringar")
brödtext(
    "Överföringar mellan dina egna Avanza-konton (t.ex. flytt från ISK till KF) "
    "räknades tidigare som insatt kapital. Det gav ett för högt nettoinsatt och ett för lågt "
    "avkastningsvärde. Nu filtreras interna överföringar bort automatiskt — "
    "nettoinsatt kapital visar enbart faktiskt nytt kapital från dig."
)

rubrik2("Tidigare versioner i korthet")
punkt("v3.14 — Ticker-sökning och automatisk MA200-hämtning via Alpha Vantage borttagen; MA200 anges manuellt.")
punkt("v3.13 — Avstämning omarbetad: layout, Avanza-stil. Buggfix: Eget fondsparande fel kontonr sedan v3.03.")
punkt("v3.12 — Kontohantering: kontonr som stabilt internt index eliminerar normNamn-ambiguitet.")
punkt("v3.11 — Ny Inställningar-sektion: kontokonfiguration, kategori-editor, strategiparametrar, profil, VP-filter.")
punkt("v3.10 — Interaktivt portföljutvecklingsdiagram, kontoregister, importordningsguide.")
