"""
Skapar Kursmaterial_Strategiportfoljen_v312.pptx
...
dst = fr"{DIR}\Kursmaterial_Strategiportfoljen_v312.pptx"
    ...
    for shape in slide.shapes:
        ...
        for para in shape.text_frame.paragraphs:
            for run in para.runs:
                for old, new in [("v3.10","v3.12"),("v310","v312"),("3.10","3.12")]:
                    if old in run.text:
...
add_tb(new_slide, MARGIN, Inches(0.25), W-2*MARGIN, Inches(0.7),
       "Nyheter i version 3.12", 26, bold=True, color=WHITE)
...
add_tb(new_slide, MARGIN, H-Inches(0.45), W-2*MARGIN, Inches(0.35),
       "Strategiportföljen — Kursmaterial · v3.12", 9, color=MUTED,
